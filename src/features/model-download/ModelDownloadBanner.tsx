...
import { ModelDownloadConsentModal } from "./ModelDownloadConsentModal";
...
export function ModelDownloadBanner() {
  ...
  const {
    status, progress, bannerDismissed, consentDeclined,
    startDownload, cancelDownload, dismissBanner, declineConsent,
  } = useBackgroundModelDownload();
  const [consentVisible, setConsentVisible] = useState(false);

  // Don't show if completed, banner dismissed, or user permanently declined
  if (status === "completed" || bannerDismissed || consentDeclined) return null;
  ...
  return (
    <>
      <ModelDownloadConsentModal
        ...
        onDecline={() => {
          setConsentVisible(false);
          declineConsent(); // sets consentDeclined + bannerDismissed
        }}
      />
...
const styles = StyleSheet.create({
  ...
  text: { flex: 1, fontSize: 13, fontFamily: "Inter_400Regular", color: INK },
  progressGroup: { flex: 1, gap: 6 },
  trackBg: { height: 4, backgroundColor: "rgba(0,30,43,0.1)", borderRadius: 2, overflow: "hidden" },
  trackFill: { height: 4, backgroundColor: ACCENT, borderRadius: 2 },
  actionBtn: { paddingHorizontal: 12, paddingVertical: 7, backgroundColor: ACCENT, borderRadius: 20 },
  actionBtnText: { fontSize: 12, fontWeight: "600", fontFamily: "Inter_600SemiBold", color: "#FFFFFF" },
  closeBtn: { padding: 4 },
});
